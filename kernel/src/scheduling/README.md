...
- every nth clock ticks, the scheduler will scan the lower queues to see if any process have been starving for too long

ISSUE: this scan for starving threads might last too long if there are a lot of tasks in the queues. TODO: review this part of the scheduler behaviour if it proves to slow down scheduling too much. an event driven approach is the only alternative that i see to this issue
