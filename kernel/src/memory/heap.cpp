...
static void expand_heap(struct heap_node *last_node, size_t requested_size, bool merge) {
    ...
    size_t heap_end = reinterpret_cast<size_t>(reinterpret_cast<uint8_t *>(last_node)
    ...
    if (merge == true) {
        ...
        new_node = reinterpret_cast<struct heap_node *> (heap_end + requested_size);
        offset = requested_size;
    } else {
        // make a new node with all of the space
        new_node = reinterpret_cast<struct heap_node *> (heap_end);
    }
    // make new node
